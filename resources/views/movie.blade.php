<x-layout>
    <div class="relative w-full h-[60vh] bg-gray-900">
        <img src="https://image.tmdb.org/t/p/original{{ $movie['backdrop_path'] }}" alt="{{ $movie['title'] }}" class="w-full h-full object-cover opacity-40">
    ...
    <main class="max-w-7xl mx-auto px-6 -mt-32 relative z-10 pb-20">
        <div class="flex flex-col md:flex-row gap-10">
            <div class="w-full md:w-1/3 lg:w-1/4 flex-shrink-0">
                <img src="https://image.tmdb.org/t/p/w500{{ $movie['poster_path'] }}" alt="{{ $movie['title'] }}" class="w-full rounded-2xl shadow-2xl border border-white/10 hover:scale-105 transition-transform duration-300">
            </div>
            ...
            <div class="w-full md:w-2/3 lg:w-3/4 pt-4 md:pt-12">
                ...
                @if($movie['tagline'])
                    <p class="text-xl text-cyan-400 italic mb-6">"{{ $movie['tagline'] }}"</p>
                @endif
                ...
                <div class="flex items-center gap-6 mb-8 text-sm md:text-base">
                    <div class="flex items-center gap-2 bg-yellow-500/10 text-yellow-400 px-4 py-2 rounded-full border border-yellow-500/20 font-bold">
                        <span>★</span> {{ number_format($movie['vote_average'], 1) }} / 10
                    </div>
                    <div class="text-gray-400">
                        {{ (int)($movie['runtime'] / 60) }}h {{ $movie['runtime'] % 60 }}m
                    </div>
                    ...
                    <div class="relative z-10">
                        @if($reviews->count() > 2)
                            <p class="text-gray-300 text-sm leading-relaxed mb-4">
                                Based on {{ $reviews->count() }} user reviews, the audience generally praises the pacing and performances. The visual effects are a standout, though some users found the third act slightly predictable. Overall, it's a solid watch!
                            </p>
