    ...
    <main class="pb-16 px-6">
        <div class="max-w-4xl mx-auto text-center mt-12 md:mt-24">
            ...
            <form action="{{ route('movie.search') }}" method="GET" class="relative max-w-2xl mx-auto group">
                ...
                <input type="text" name="query" placeholder="Search for a movie, book, or artist..." required class="relative w-full bg-[#18181b] text-white px-8 py-5 rounded-full text-lg border border-white/10 focus:border-cyan-500/50 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 placeholder-gray-500" />
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            ...
            <a href="/movie/{{ $movie['id'] }}" class="group cursor-pointer block">
                <div class="relative overflow-hidden rounded-2xl aspect-[2/3] bg-gray-800">
                    <img src="https://image.tmdb.org/t/p/w500{{ $movie['poster_path'] }}" alt="{{ $movie['title'] ?? $movie['name'] }}" class="object-cover w-full h-full group-hover:scale-110 transition-transform duration-300" />
                ...
                <p class="text-sm text-gray-500">
                    {{ isset($movie['release_date']) ? \Carbon\Carbon::parse($movie['release_date'])->format('Y') : (isset($movie['first_air_date']) ? \Carbon\Carbon::parse($movie['first_air_date'])->format('Y') : 'N/A') }}
                </p>
            </a>
            @endforeach
