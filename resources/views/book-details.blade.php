<x-layout>
    ...
    <div class="relative w-full h-[50vh] bg-gray-900 overflow-hidden">
        @if($cover)
            <img src="{{ $cover }}" alt="{{ $title }}" class="w-full h-full object-cover opacity-20 blur-xl scale-110">
        @endif
    ...
    <main class="max-w-7xl mx-auto px-6 -mt-40 relative z-10 pb-20">
        <div class="flex flex-col md:flex-row gap-10">
            ...
            <div class="w-full md:w-1/3 lg:w-1/4 flex-shrink-0">
                @if($cover)
                    <img src="{{ $cover }}" alt="{{ $title }}" class="w-full rounded-2xl shadow-2xl border border-white/10 hover:scale-105 transition-transform duration-300">
                @else
            ...
            <div class="w-full lg:w-2/3">
                ...
                @auth
                    <form action="{{ route('reviews.store') }}" method="POST" class="bg-[#18181b] p-6 rounded-2xl border border-white/10 mb-10 shadow-lg">
                        @csrf
                        <input type="hidden" name="media_id" value="{{ $bookId }}">
                ...
                @else
                    <div class="bg-[#18181b] p-6 rounded-2xl border border-white/10 mb-10 text-center">
                        ...
                        <a href="{{ route('login') }}" class="inline-block bg-emerald-500 text-black font-bold py-2 px-6 rounded-full hover:scale-105 transition-transform text-sm">Log in to Review</a>
                    </div>
                @endauth
